...
class EducationDB:
    ...
    def get_course(self, title: str) -> Optional[Dict]:
    ...
    def add_task(self, course_title: str, task_title: str, 
        ...
        with self._get_connection() as con:
            ...
            try:
                con.execute(
                    """INSERT INTO tasks 
                    (course_id, title, description, input_data, output_data, demand_list)
                    VALUES (?, ?, ?, ?, ?, ?);""",
                    (course[0], task_title, description, input_data, output_data, demand_list)
                )
                return True
            except sq.IntegrityError:
                return False
    ...
    def get_course_tasks(self, course_title: str) -> List[Dict]:
        ...
        with self._get_connection() as con:
            cursor = con.cursor()
            cursor.execute("""
                SELECT t.task_id, t.title, t.description, t.input_data, t.output_data, t.demand_list
                FROM tasks t
    ...
    def get_task(self, task_name: str, course_name: str) -> Optional[Dict]:
        """Возвращает задание по имени задания и названию курса."""
        with self._get_connection() as con:
            cursor = con.cursor()
            cursor.execute("""
                ...
                FROM tasks t
                JOIN courses c ON t.course_id = c.course_id
                WHERE t.title = ? AND c.title = ?;
            """, (task_name, course_name))  #  Ищем по имени задания и названию курса
            row = cursor.fetchone()
